...
**`KAKAO_NATIVE_KEY`를 생략해도 된다.** 비어 있으면 랜딩의 카카오 버튼이
개발용 로그인(`POST /auth/test/login`, 계정 `doyoon@iam.app`)으로 폴백하고,
화면 하단에 그 사실이 표시된다. 2·3·4단계 없이 서버 연동만 먼저 확인할 때 이 경로를 쓴다.
...
| 세션 없음 | 랜딩으로 이동 |
| "신규 회원으로 둘러보기" | 서버에 새 계정 생성 → 온보딩으로 이동(빈 화면 — 아직 미구현) |
| 기존 계정 로그인 | 홈으로 이동 → **모임 카드 목록이 보이면 서버 연동 성공** |
